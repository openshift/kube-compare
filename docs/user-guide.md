...
#### Specifying Template match choosing mechanism in case of two templates with same group

In cases where the reference includes two templates with the same apiVersion-kind-namespace-name 4-tuple, by the correlation
logic that was explained in previous section the command will diff with the template that is best matched to the CR.
The best match behaviour can be configured in the diff config. The default behaviour is to use the template with the least
number of lines in the diff with the CR (Named 'NumLines'). Is also possible to disable this mechanism by specifying 'None'
under the bestMatchType section in the user config. The only supported values are: NumLines and None. More mechanisms can
be added by request.
...
```yaml
bestMatchType: "NumLines" # Default no need to specify 
```
