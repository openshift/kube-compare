...
FROM registry.ci.openshift.org/ocp/builder:rhel-8-golang-1.24-openshift-4.19 AS builder-rhel-8
WORKDIR /go/src/github.com/openshift/kube-compare
COPY . .
RUN make cross-build --warn-undefined-variables

FROM registry.ci.openshift.org/ocp/builder:rhel-9-golang-1.24-openshift-4.19 AS builder-rhel-9
WORKDIR /go/src/github.com/openshift/kube-compare
COPY . .
RUN make cross-build --warn-undefined-variables

FROM registry.ci.openshift.org/ocp/4.19:cli AS final-builder
